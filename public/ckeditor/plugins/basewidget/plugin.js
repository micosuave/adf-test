...
function addWidget(editor, name, definition) {
    ...
    var configToolbar = function(configuration) {
        ...
        this.wrapper.$.onmouseleave = function(event) {
            ...
            setTimeout(function() {
                removeToolbar();
                if (this.wrapper) { // by the time the function is called the widget might be removed
                    var widgetParents = this.wrapper.getParents(true);
                    for (var i = 1; i < widgetParents.length; i++) { // start from 1 because the widget itself is on 0
                        if (widgetParents[i].hasClass('cke_widget_wrapper')) {
                            widgetParents[i].$.onmouseenter();
                            break;
                        }
                    }
                }
